/*
 **
 ** EMF Machine : auto-gen : jupiterace
 **
 */
function emfjupiterace(options) {
  options = options || {};
  ...
  m.device.cpu.emulate = new jupiterace_z80_emulator(m.bus, options.cpu);
  m.device.cpu.disassemble = new jupiterace_z80_disassemble(m.bus, options.cpu);
  m.device.cpu.assemble = new jupiterace_z80_assemble(m.bus, options.cpu);
  m.device.memory = new jupiterace_memory(m.bus, options.memory);
  m.device.display = new jupiterace_display(m.bus, options.display);
  m.device.iorq = new jupiterace_iorq(m.bus, options.iorq);
  ...
  m.bus.clock = {};
  m.clock = {};
  m.clock.cpu = new jupiterace_clock_cpu(m, options);
  m.bus.clock.cpu = m.clock.cpu;
  m.clock.vsync = new jupiterace_clock_vsync(m, options);
  m.bus.clock.vsync = m.clock.vsync;
  ...
  m.start = function() {
    ...
    if (m.clock.vsync.start) m.clock.vsync.start(m.clock.vsync, arguments);
    return processed;
  };
  m.reset = function() {
    let processed = {};
    ...
    if (m.clock.vsync.reset) m.clock.vsync.reset(m.clock.vsync, arguments);
    return processed;
  };
  m.getState = function() {
    let processed = {};
    ...
    if (m.clock.vsync.getState) m.clock.vsync.getState(m.clock.vsync, arguments);
    return processed;
  };
  m.setState = function() {
    let processed = {};
    ...
    if (m.clock.vsync.setState) m.clock.vsync.setState(m.clock.vsync, arguments);
    return processed;
  };
