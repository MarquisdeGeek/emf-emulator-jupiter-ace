...
function SGXPrepare_OS() {
  ...
  gStateVars.machine = new emfjupiterace(options)
...
function uiConfigure(m) {
  ...
  $('#emf_ace_3k').click(function(ev) {
    stopRunning();
    m.bus.memory.disableBlock('ram16k');
    m.bus.pulseLow('reset');
    startRunning();
  });

  $('#emf_ace_19k').click(function(ev) {
    stopRunning();
    m.bus.memory.enableBlock('ram16k');
    m.bus.pulseLow('reset');
    startRunning();
  });
...
function importMachineData(machine, filename, data) {
  ...
  // FILE: ui/web/importer.js
  // PURPOSE: Add the implementation of the 'importMachineData' method to correctly
  // load general-purpose and machine-specific snapshots.
  // NOTE: Use 'filename' to determine the type. 'data' contains a plain array

  if (filename.toLowerCase().indexOf('.ace') !== -1) {
    let aceimporter = new ace.importer(gStateVars.machine);
    aceimporter.ace(filename, data);
  } else {
